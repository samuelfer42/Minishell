...
{
	tok->data = malloc(datasize + 1); // 1 pour le terminateur null
	tok->data[0] = 0;
...
int lexer_build(char* input, int size, lexer_t* lexerbuf)
{
		...
		if (state == STATE_GENERAL)
		{
			switch (chtype) 
			{
				...
				case CHAR_PIPE:
					...
					// termine le token qui était en train d'être lu avant
					if (j > 0) {
						token->data[j] = 0;
						token->next = malloc(sizeof(tok_t));
						token = token->next;
	...
	while (token != NULL) 
	{
		if (token->type == TOKEN)
		{
			...
			{
				k += globbuf.gl_pathc;
				// sauvegarder le prochain token pour pouvoir l'attacher plus tard
				tok_t* saved = token->next;
				
				// remplacer le token actuel par le premier token
				free(token->data);
				...
				// Le token de l'utilisateur peut être entre guillemets pour échapper aux caractères spéciaux.
				// d'où la suppression du guillemet
