...
/*** Grammaire shell pour analyseur descendant récursif ***/
/*** Suppression de la récursion gauche et de la factorisation gauche ***/
 ...
 *
	...
	<command>		::=		<simple command> '<' <filename> // cette grammaire est un peu incorrecte, voir grammer.llf
						|	<simple command> '>' <filename>
						|	<simple command>
...
ASTreeNode* CMDLINE();		//	tester toutes les productions en ligne de commande dans l'ordre
...
ASTreeNode* JOB();			// tester la production de tous les job en ordre
ASTreeNode* JOB1();			// <command> '|' <job>
ASTreeNode* JOB2();			// <command>

ASTreeNode* CMD();			// tester la production de toutes les commandes dans l'ordre
...
ASTreeNode* SIMPLECMD1()
{
    ...
    // nous ne vérifions pas si tokenlistNode est NULL puisque c'est une grammaire valide.
